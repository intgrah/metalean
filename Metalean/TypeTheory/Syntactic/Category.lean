...
abbrev snoc (Γ : RawCtx E ℓ) {t : Expr ζ ℓ Γ.len} (ht : E[Γ.ctx] ⊢ t typ) : RawCtx E ℓ where
  ctx := Γ.ctx.snoc t
  wf := Γ.wf.snoc ht

structure Hom (Γ₁ Γ₂ : RawCtx E ℓ) where
  subst : Subst ζ ℓ Γ₂.len Γ₁.len
  typed : E[Γ₁.ctx] ⊢ subst ⊣ Γ₂.ctx
...
def snoc (σ : Γ₁ ⟶ Γ₂) {e : Expr ζ ℓ Γ₁.len} {t : Expr ζ ℓ Γ₂.len}
    (ht : E[Γ₂.ctx] ⊢ t typ)
    (he : E[Γ₁.ctx] ⊢ e : t.subst σ.subst) :
    Γ₁ ⟶ Γ₂.snoc ht where
  subst := σ.subst.extend e
  typed := σ.typed.extend he

theorem exists_snoc {t : Expr ζ ℓ Γ₂.len} (ht : E[Γ₂.ctx] ⊢ t typ)
    (σ : Γ₁ ⟶ Γ₂.snoc ht) :
    ∃ (τ : Γ₁ ⟶ Γ₂) (e : Expr ζ ℓ Γ₁.len) (he : E[Γ₁.ctx] ⊢ e : t.subst τ.subst),
      σ = τ.snoc ht he :=
  let τ : Γ₁ ⟶ Γ₂ := ⟨fun v => σ.subst v.castSucc, (SubstWF.wk t Γ₂.wf).comp σ.typed⟩
  have he : E[Γ₁.ctx] ⊢ σ.subst (Fin.last Γ₂.len) : t.subst τ.subst := by
    have h := σ.typed (Fin.last Γ₂.len)
    rw [Ctx.get_last, Expr.wk_subst] at h
    exact h
  ⟨τ, _, he, Hom.ext (Fin.snoc_init_self σ.subst).symm⟩

def one {t e : Expr ζ ℓ Γ₂.len} (ht : E[Γ₂.ctx] ⊢ t typ) (he : E[Γ₂.ctx] ⊢ e : t) :
    Γ₂ ⟶ Γ₂.snoc ht where
  subst := Subst.id.extend e
  typed := SubstWF.inst Γ₂.wf he

def lift (σ : Γ₁ ⟶ Γ₂) {t : Expr ζ ℓ Γ₂.len} (ht : E[Γ₂.ctx] ⊢ t typ) :
...
@[simp] theorem snoc_subst (σ : Γ₁ ⟶ Γ₂) {t : Expr ζ ℓ Γ₂.len} (ht : E[Γ₂.ctx] ⊢ t typ)
    {e : Expr ζ ℓ Γ₁.len} (he : E[Γ₁.ctx] ⊢ e : t.subst σ.subst) :
    (σ.snoc ht he).subst = σ.subst.extend e := rfl

@[simp] theorem one_subst {t e : Expr ζ ℓ Γ₂.len} (ht : E[Γ₂.ctx] ⊢ t typ)
    (he : E[Γ₂.ctx] ⊢ e : t) :
...
instance : Congruence (homRel E ℓ) where
  ...
  comp_right {_ Y _} {_ _} σ h := SubstEq.comp Y.wf σ.typed h
...
theorem RawCtx.toCtx_map_eq_iff {Γ₁ Γ₂ : RawCtx E ℓ} (σ₁ σ₂ : Γ₁ ⟶ Γ₂) :
    toCtx.map σ₁ = toCtx.map σ₂ ↔ E[Γ₁.ctx] ⊢ σ₁.subst ≡ σ₂.subst ⊣ Γ₂.ctx :=
