...
abbrev Raw.ContextSection {t : Expr ζ ℓ Γ₁.as.len} {u : Level ℓ} (ht : E[Γ₁.as.ctx] ⊢ t : .sort u)
    (σ : Γ₂ ⟶ Γ₁) (label : Tm_ Γ₂) :=
...
def ofTerm {Γ : CtxCat E ℓ} {t e : Expr ζ ℓ Γ.as.len} {u : Level ℓ}
  ...
  over := (snoc_projection Γ ht (𝟙 Γ.as)
    (congr(E[Γ.as.ctx] ⊢ e : $(RawCtx.expr.map_id_apply (Opposite.op Γ.as) t)).mpr he)).trans
...
def mapExtension (ht : E[Γ₂.as.ctx] ⊢ t : .sort u) (σ₁ : Γ₁.as ⟶ Γ₂.as)
    {σ₂ : Γ₃ ⟶ Γ₁} {label : Tm_ Γ₃}
...
def cartesianLift (ht : E[Γ₂.as.ctx] ⊢ t : .sort u) (σ₁ : Γ₁.as ⟶ Γ₂.as)
    {σ₂ : Γ₃ ⟶ Γ₁} {label : Tm_ Γ₃}
...
def convert (h : E[Γ₁.as.ctx] ⊢ t₁ ≡ t₂ : .sort u) {σ : Γ₂ ⟶ Γ₁}
