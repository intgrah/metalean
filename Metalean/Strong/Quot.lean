...
variable {η : Head ζ .quot} {v : Level ℓ} {α r β f h q p : Expr ζ ℓ n}

theorem DefeqStrong.quot_prop_witness :
    E[Γ] ⊢ₛ α : .prop →
    E[Γ] ⊢ₛ r : Quot.relType α →
    E[Γ] ⊢ₛ q : .quot η .zero α r →
    E[Γ] ⊢ₛ .quotInd η .zero α r (.lam (.quot η .zero α r) α.wk)
      (.lam α (.var (Fin.last n))) q : α := by
  intro hα hr hq
  have hquot : E[Γ] ⊢ₛ .quot η .zero α r : .prop := .quotDF hα hr
  have hαwkQ : E[Γ.snoc (.quot η .zero α r)] ⊢ₛ α.wk : .prop :=
    hα.wk _
  have hmotive : E[Γ] ⊢ₛ .lam (.quot η .zero α r) α.wk : Quot.motiveType η .zero α r :=
    .lamDF hquot .sortDF .sortDF hαwkQ hαwkQ
  have hαwk : E[Γ.snoc α] ⊢ₛ α.wk : .prop := hα.wk α
  have hrwk : E[Γ.snoc α] ⊢ₛ r.wk : Quot.relType α.wk := by
    simpa [Expr.wk] using hr.wk α
  have hvar : E[Γ.snoc α] ⊢ₛ .var (Fin.last n) : α.wk := by
    have := DefeqStrong.var (Γ := Γ.snoc α) (v := Fin.last n) (by rw [Ctx.get_last]; exact hαwk)
    rwa [Ctx.get_last] at this
  have hquotwk : E[Γ.snoc α] ⊢ₛ .quot η .zero α.wk r.wk : .prop := hquot.wk α
  have hbody : E[(Γ.snoc α).snoc (.quot η .zero α.wk r.wk)] ⊢ₛ α.wk.wk : .prop :=
    hαwk.wk _
  have hmk : E[Γ.snoc α] ⊢ₛ Quot.minorQuotMk η .zero α r : .quot η .zero α.wk r.wk :=
    .quotMkDF hαwk hrwk hvar
  have hbeta := DefeqStrong.beta (t' := .prop) hquotwk .sortDF hbody hmk .sortDF
    (by rw [Expr.inst_wk]; exact hαwk)
  rw [Expr.inst_wk] at hbeta
  have hmotiveWk : (Expr.lam (.quot η .zero α r) α.wk).wk =
      .lam (.quot η .zero α.wk r.wk) α.wk.wk := by
    simp [Expr.wk]
  have hminorType : E[Γ] ⊢ₛ .forallE α α.wk ≡
      Quot.minorType η .zero α r (.lam (.quot η .zero α r) α.wk) : .sort (.imax .zero .zero) := by
    unfold Quot.minorType
    rw [hmotiveWk]
    exact .forallEDF hα hbeta.symm hbeta.symm
  have hminor := DefeqStrong.defeqDF hminorType (.lamDF hα hαwk hαwk hvar hvar)
  have hresult : E[Γ] ⊢ₛ .app (.lam (.quot η .zero α r) α.wk) q : .prop :=
    .appDF (t' := .prop) hquot .sortDF hmotive hq .sortDF
  have hbetaQ := DefeqStrong.beta (t' := .prop) hquot .sortDF hαwkQ hq .sortDF
    (by rw [Expr.inst_wk]; exact hα)
  rw [Expr.inst_wk] at hbetaQ
  exact .defeqDF hbetaQ (.quotIndDF hα hr hmotive hminor hq hresult)

theorem DefeqStrong.quotLift_prop :
    E[Γ] ⊢ₛ α : .prop →
    E[Γ] ⊢ₛ r : Quot.relType α →
    E[Γ] ⊢ₛ β : .sort v →
    E[Γ] ⊢ₛ f : .forallE α β.wk →
    E[Γ] ⊢ₛ h : Quot.compatType (E.get η).eqHead v α r β f →
    E[Γ] ⊢ₛ q : .quot η .zero α r →
    E[Γ] ⊢ₛ p : α →
    E[Γ] ⊢ₛ .quotLift η .zero v α r β f h q ≡ .app f p : β := by
  intro hα hr hβ hf hh hq hp
