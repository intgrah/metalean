...
theorem projTypeWith_hasType
    (hctor : CtorWF E I (I.ctors s c))
    {f : Fin (ι.ctors s c).nfields}
    {previous : Fin f.val → Expr ζ ℓ n}
    (hps : ∀ p, E[Γ] ⊢ ps p : I.paramType ls ps p)
    (hprevious : ∀ prior, E[Γ] ⊢ previous prior :
...
theorem projTypeWith_congr
    ...
    (hps : ∀ p, E[Γ] ⊢ ps p : I.paramType ls ps p)
    (hprevious : ∀ prior, E[Γ] ⊢ previous prior ≡ previous' prior :
      projTypeWith I ls ps
        (prior.castLE f.isLt.le)
        fun earlier => previous
          (earlier.castLE prior.isLt.le)) :
  ...
  have hfullEq := SubstEq.extendFamily hfieldsTele hpsEq fun prior => by
    rw [← Ctx.entry_instL]
    simpa [projTypeWith] using hprevious prior
...
theorem wkParams
    ...
    (p : Fin ι.nparams) :
    E[Γ.snoc (.ind η s ls ps h.indices)] ⊢ (ps p).wk :
      (E.get η).block.paramType ls (fun p => (ps p).wk) p := by
  simpa [Expr.wk] using (hps p).wk (.ind η s ls ps h.indices)

theorem varMajor (hΓ : E[Γ] ⊢ ok)
    (hps : ∀ p, E[Γ] ⊢ ps p : (E.get η).block.paramType ls ps p) :
    E[Γ.snoc (.ind η s ls ps h.indices)] ⊢ .var (Fin.last n) :
      .ind η s ls (fun p => (ps p).wk) h.indices := by
  have hΓ' : E[Γ.snoc (.ind η s ls ps h.indices)] ⊢ ok :=
    hΓ.snoc ⟨_, h.indType hps⟩
...
theorem projection_spec {n : Nat} {Γ : Ctx ζ ℓ 0 n} {ps : Fin ι.nparams → Expr ζ ℓ n}
    {maj : Expr ζ ℓ n}
    (hΓ : E[Γ] ⊢ ok)
    (hps : ∀ p, E[Γ] ⊢ ps p : (E.get η).block.paramType ls ps p)
    (hmaj : E[Γ] ⊢ maj : .ind η s ls ps h.indices)
    (f : Fin (ι.ctors s c).nfields) :
  ...
  have hbody {m : Nat} {Δ : Ctx ζ ℓ 0 m} {qs : Fin ι.nparams → Expr ζ ℓ m}
      {t : Expr ζ ℓ m} (hΔ : E[Δ] ⊢ ok)
      (hqs : ∀ p, E[Δ] ⊢ qs p : (E.get η).block.paramType ls qs p)
      (ht : E[Δ] ⊢ t : .ind η s ls qs h.indices) :
      E[Δ] ⊢ h.projType η ls qs f t : .sort u := by
    rw [projType_eq_projTypeWith]
    refine projTypeWith_hasType (hB.ctors s c) hqs fun prior => ?_
    have hp := (projection_spec hΔ hqs ht (prior.castLT (prior.isLt.trans f.isLt))).term
    rw [projType_eq_projTypeWith] at hp
    exact hp
  have hsnoc {m : Nat} {Δ : Ctx ζ ℓ 0 m} {qs : Fin ι.nparams → Expr ζ ℓ m}
      (hΔ : E[Δ] ⊢ ok)
      (hqs : ∀ p, E[Δ] ⊢ qs p : (E.get η).block.paramType ls qs p) :
      E[Δ.snoc (.ind η s ls qs h.indices)] ⊢
        h.projType η ls (fun p => (qs p).wk) f (.var (Fin.last m)) : .sort u :=
    hbody (hΔ.snoc ⟨_, h.indType hqs⟩) (h.wkParams hqs) (h.varMajor hΔ hqs)
  have hbeta {m : Nat} {Δ : Ctx ζ ℓ 0 m} {qs : Fin ι.nparams → Expr ζ ℓ m}
      {t : Expr ζ ℓ m} (hΔ : E[Δ] ⊢ ok)
      (hqs : ∀ p, E[Δ] ⊢ qs p : (E.get η).block.paramType ls qs p)
      (ht : E[Δ] ⊢ t : .ind η s ls qs h.indices) :
      E[Δ] ⊢ .app (h.projectionMotives η ls qs f s) t ≡
        h.projType η ls qs f t : .sort u := by
    rw [h.projectionMotives_self]
    have hb := Defeq.beta (h.indType hqs) .sortDF (hsnoc hΔ hqs) ht
      (by simpa [Expr.inst] using Defeq.sortDF)
      ((hsnoc hΔ hqs).substitution (SubstWF.inst hΔ ht.left))
    convert hb using 1
  ...
  have hiotaType {m : Nat} {Δ : Ctx ζ ℓ 0 m} {qs : Fin ι.nparams → Expr ζ ℓ m}
      {gds : Fin (ι.ctors s c).nfields → Expr ζ ℓ m} (hΔ : E[Δ] ⊢ ok)
      (hqs : ∀ p, E[Δ] ⊢ qs p : (E.get η).block.paramType ls qs p)
      (ht : E[Δ] ⊢ .ctor η s c ls qs gds h.recursive :
        .ind η s ls qs h.indices)
      (hgds : ∀ current, E[Δ] ⊢ gds current :
        projTypeWith ((E.get η).block) ls qs current fun previous =>
          gds (previous.castLE current.isLt.le)) :
      E[Δ] ⊢ h.projType η ls qs f (.ctor η s c ls qs gds h.recursive) ≡
        projTypeWith ((E.get η).block) ls qs f
          (fun prior => gds (prior.castLE f.isLt.le)) : .sort u := by
    rw [projType_eq_projTypeWith]
    refine projTypeWith_congr hB (hB.ctors s c) hqs fun prior => ?_
    have hp := (projection_spec hΔ hqs ht
      (prior.castLT (prior.isLt.trans f.isLt))).iota gds rfl hgds
    rw [projType_eq_projTypeWith] at hp
    exact hp
  have hmotive : E[Γ] ⊢ ms s : (E.get η).block.motiveType η ls ps u s := by
    rw [hmsDef, h.projectionMotives_self, h.motiveType_self]
    exact .lamDF (h.indType hps) .sortDF .sortDF (hsnoc hΓ hps) (hsnoc hΓ hps)
  have hms : ∀ target, E[Γ] ⊢ ms target :
  ...
  have hΓcase := (hB.caseTele (c := c) hΓ hps hms).appendCtxWF hΓ
  ...
  have hcaseRec : ((ι.ctors s c).caseRecursive :
    ...
    h.recursive_eq _
  have hcaseMajor : E[Γ ++ (E.get η).block.caseTele η ls ps ms s c] ⊢
    ...
    exact .ctorDF (recFieldLevels := fun _ => .zero)
      ...
      h.no_recursive.elim
      (fun current => (hB.ctors s c).ordinaryFieldExpr current hcaseParams hcaseOrdinary)
      h.no_recursive.elim
  ...
  have hcaseMotive : (((ms s).wkN (ι.ctors s c).nfields).wkN
    ...
    rfl
  have hcaseBeta : E[Γ ++ (E.get η).block.caseTele η ls ps ms s c] ⊢
  ...
  have htargetIdx : (fun i =>
    ...
    h.indices_eq _
  have htypeIota : E[Γ] ⊢ (E.get η).block.iotaType η ls ps ms s c fds h.recursive ≡
        h.projType η ls ps f (.ctor η s c ls ps fds h.recursive) : .sort u := by
    simpa [Inductive.iotaType, htargetIdx] using hresult
  have hlhs : E[Γ] ⊢ (E.get η).block.iotaLhs η ls u ps ms mins s c fds h.recursive :
      (E.get η).block.iotaType η ls ps ms s c fds h.recursive := by
    have ht := Defeq.defeqDF htypeIota.symm hterm
    rw [h.projTerm_eq_recr, ← htargetIdx] at ht
    exact ht
  have hbodyOrd : E[Γ ++ ((E.get η).block.ctors s c).ordinaryFieldTele η ls ps] ⊢
    ...
    have hv := (hB.ctors s c).boundOrdinarySubstWF (η := η) hps (Fin.natAdd ι.nparams f)
  ...
  have hrhsEq : E[Γ] ⊢ (E.get η).block.iotaRhs η ls u ps ms mins s c fds h.recursive ≡ fds f :
      projTypeWith ((E.get η).block) ls ps f fun previous =>
        fds (previous.castLE f.isLt.le) := by
    have happly := Ctx.lam_applyFamily hΓ hfieldsTele
      (fun current => by
...
theorem projTerm_hasType :
    E[Γ] ⊢ ok →
    (∀ p, E[Γ] ⊢ ps p : (E.get η).block.paramType ls ps p) →
    E[Γ] ⊢ maj : .ind η s ls ps h.indices →
    E[Γ] ⊢ h.projTerm η ls ps f maj :
      h.projType η ls ps f maj :=
  fun hΓ hps hmaj => (h.projection_spec hB hΓ hps hmaj f).term

theorem projType_hasType :
    E[Γ] ⊢ ok →
    (∀ p, E[Γ] ⊢ ps p : (E.get η).block.paramType ls ps p) →
    E[Γ] ⊢ maj : .ind η s ls ps h.indices →
...
theorem projTerm_generic :
    E[Γ] ⊢ ok →
    (∀ p, E[Γ] ⊢ ps p : (E.get η).block.paramType ls ps p) →
    E[Γ.snoc (.ind η s ls ps h.indices)] ⊢
      h.projTerm η ls (fun p => (ps p).wk) f (.var (Fin.last n)) :
      h.projType η ls (fun p => (ps p).wk) f (.var (Fin.last n)) :=
  fun hΓ hps =>
    h.projTerm_hasType hB f (hΓ.snoc ⟨_, h.indType hps⟩) (h.wkParams hps) (h.varMajor hΓ hps)

theorem projTerm_substCongr {m : Nat} {Δ : Ctx ζ ℓ 0 m}
  ...
  simpa [Expr.wk_subst_extend, Expr.subst, Subst.extend_last] using
    Defeq.substitution_congr hΓ' hξ (h.projTerm_generic hB f hΓ hps)

theorem projTerm_congr :
    (∀ p, E[Γ] ⊢ ps₁ p ≡ ps₂ p : (E.get η).block.paramType ls ps₁ p) →
    E[Γ] ⊢ maj₁ ≡ maj₂ : .ind η s ls ps₁ h.indices →
    E[Γ] ⊢ h.projTerm η ls ps₁ f maj₁ ≡ h.projTerm η ls ps₂ f maj₂ :
      h.projType η ls ps₁ f maj₁ := by
  intro hps hmaj
  ...
  have hgeneric : ∀ p : Fin ι.nparams,
    ...
    exact hvar
  exact h.projTerm_substCongr hB f hΓ₀ hgeneric (Inductive.paramSubstEq hps) hmaj

theorem projType_congr :
    (∀ p, E[Γ] ⊢ ps₁ p ≡ ps₂ p : (E.get η).block.paramType ls ps₁ p) →
    E[Γ] ⊢ maj₁ ≡ maj₂ : .ind η s ls ps₁ h.indices →
    E[Γ] ⊢ h.projType η ls ps₁ f maj₁ ≡ h.projType η ls ps₂ f maj₂ typ := by
  intro hps hmaj
  have hfields : ∀ current : Fin (ι.ctors s c).nfields,
    ...
    intro current
    have hcongr := h.projTerm_congr hB current hps hmaj
...
theorem rebuildTerm_hasType (is : Fin (ι.nindices s) → Expr ζ ℓ n) :
    E[Γ] ⊢ ok →
    (∀ p, E[Γ] ⊢ ps p : (E.get η).block.paramType ls ps p) →
    E[Γ] ⊢ maj : .ind η s ls ps h.indices →
  ...
  have hfields : ∀ f, E[Γ] ⊢ h.projTerm η ls ps f maj :
    ...
    intro f
    have hf := h.projTerm_hasType hB f hΓ hps hmaj
    rw [projType_eq_projTypeWith] at hf
    exact hf
  have hrebuild := Defeq.ctorDF
    (recFds₁ := h.recursive) (recFds₂ := h.recursive)
    (recFieldLevels := h.no_recursive.elim)
    hps hfields
    h.no_recursive.elim
    (fun f => (hB.ctors s c).ordinaryFieldExpr f hps hfields)
    h.no_recursive.elim
    (Defeq.indDF hps h.no_indices.elim)
...
theorem projTerm_ctor (fds : Fin (ι.ctors s c).nfields → Expr ζ ℓ n) :
    E[Γ] ⊢ ok →
    (∀ p, E[Γ] ⊢ ps p : (E.get η).block.paramType ls ps p) →
    E[Γ] ⊢ .ctor η s c ls ps fds h.recursive :
      .ind η s ls ps h.indices →
    (∀ current, E[Γ] ⊢ fds current :
      projTypeWith ((E.get η).block) ls ps current fun previous =>
        fds (previous.castLE current.isLt.le)) →
    E[Γ] ⊢ h.projTerm η ls ps f (.ctor η s c ls ps fds h.recursive) ≡ fds f :
      h.projType η ls ps f (.ctor η s c ls ps fds h.recursive) :=
  fun hΓ hps hmaj => (h.projection_spec hB hΓ hps hmaj f).iota fds rfl
