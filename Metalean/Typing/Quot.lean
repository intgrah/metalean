...
theorem relType_congr :
    E[Γ] ⊢ α ≡ α' : .sort u →
    E[Γ] ⊢ relType α ≡ relType α' : .sort (.imax u (.imax u .one)) :=
  fun hα =>
    .forallEDF hα
...
theorem Quot.motiveType_isType :
    E[Γ] ⊢ α : .sort l →
    E[Γ] ⊢ r : Quot.relType α →
    E[Γ] ⊢ Quot.motiveType η l α r typ :=
  fun hα hr => ⟨_, .forallEDF (.quotDF hα hr) .sortDF .sortDF⟩

theorem Quot.minorType_isType :
    E[Γ] ⊢ α : .sort l →
    E[Γ] ⊢ r : Quot.relType α →
    E[Γ] ⊢ β : Quot.motiveType η l α r →
    E[Γ] ⊢ Quot.minorType η l α r β typ := by
  intro hα hr hβ
  ...
  have hquotwk : E[Γ.snoc α] ⊢ .quot η l α.wk r.wk : .sort l := .quotDF hαwk hrwk
  have hmk : E[Γ.snoc α] ⊢ Quot.minorQuotMk η l α r : .quot η l α.wk r.wk :=
    .quotMkDF hαwk hrwk hvar
  have hβwk : E[Γ.snoc α] ⊢ β.wk : .forallE (.quot η l α.wk r.wk) .prop := by
    simpa [Quot.motiveType, Expr.wk] using hβ.wk α
  have happ : E[Γ.snoc α] ⊢ .app β.wk (Quot.minorQuotMk η l α r) : .prop :=
...
theorem Quot.compatType_isType {ηeq : Head ζ (.inductive Eq.sig)}
    (hEq : (E.get ηeq).block = Eq.block) {u l : Level ℓ} {α r β f : Expr ζ ℓ n} :
    E[Γ] ⊢ α : .sort u →
    E[Γ] ⊢ r : Quot.relType α →
    E[Γ] ⊢ β : .sort l →
    E[Γ] ⊢ f : .forallE α β.wk →
    E[Γ] ⊢ Quot.compatType ηeq l α r β f typ := by
  intro hα hr hβ hf
  have hα₁ : E[Γ.snoc α] ⊢ α.wk : .sort u := hα.wk α
  have hα₂ : E[(Γ.snoc α).snoc α.wk] ⊢ α.wk.wk : .sort u := hα₁.wk α.wk
  have ha₁ : E[Γ.snoc α] ⊢ .var (Fin.last n) : α.wk := by
    have := Defeq.var (Γ := Γ.snoc α) (v := Fin.last n) (by rw [Ctx.get_last]; exact hα₁)
    rwa [Ctx.get_last] at this
  have ha₂ : E[(Γ.snoc α).snoc α.wk] ⊢ .var ⟨n, by omega⟩ : α.wk.wk := by
    have h := ha₁.wk α.wk
    have hv : (Expr.var (Fin.last n) : Expr ζ ℓ (n + 1)).wk = .var ⟨n, by omega⟩ := by
      simp [Expr.wk, Expr.wkFrom, Expr.rename]
      exact Fin.ext rfl
    rwa [hv] at h
  have hb₂ : E[(Γ.snoc α).snoc α.wk] ⊢ .var ⟨n + 1, by omega⟩ : α.wk.wk := by
    have := Defeq.var (Γ := (Γ.snoc α).snoc α.wk) (v := Fin.last (n + 1))
  ...
  have h₁ := Defeq.appDF hα₂ hcod hr₂ ha₂ (hcod.inst_congr ha₂)
  have h₁' : E[(Γ.snoc α).snoc α.wk] ⊢ r.wk.wk.app (.var ⟨n, by omega⟩) :
  ...
  have hfa := Defeq.appDF hα₃ hβ₄ hf₃' ha₃ (hβ₄.inst_congr ha₃)
  have hfb := Defeq.appDF hα₃ hβ₄ hf₃' hb₃ (hβ₄.inst_congr hb₃)
  rw [Expr.inst_wk] at hfa hfb
  have heq := Quot.eqApp_typed hEq hβ₃ hfa hfb
  have h₃ := Defeq.forallEDF hP heq heq
  have h₂ := Defeq.forallEDF hα₁ h₃ h₃
