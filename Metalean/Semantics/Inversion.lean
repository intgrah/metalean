...
theorem Defeq.ind_model_inv {ι : IndSig}
  ...
  have heq := (h.rawSoundness hE hΔ).equal (𝟙 _) (fun _ ↦ ⊥) (hΔ.bottom_admissible hE (𝟙 _))
...
theorem TypeEq.rawInterpret_eq (hΔ : E[Δ] ⊢ ok)
    {t t' : Expr ζ ℓ n} :
    EnvWF E →
    E[Δ] ⊢ t ≡ t' typ →
  ...
  | single h =>
    have ⟨_, h⟩ := h
    exact (h.rawSoundness hE hΔ).equal (𝟙 _) (fun _ ↦ ⊥)
      (hΔ.bottom_admissible hE (𝟙 _))
  | trans _ _ ih₁ ih₂ => exact ih₁.trans ih₂

theorem TypeEq.sort_model_inj (hE : EnvWF E) (hΔ : E[Δ] ⊢ ok) {l₁ l₂ : Level ℓ}
    (h : E[Δ] ⊢ .sort l₁ ≡ .sort l₂ typ) : l₁ = l₂ := by
  have heq := h.rawInterpret_eq hΔ hE
...
theorem TypeEq.forallE_model_inj
